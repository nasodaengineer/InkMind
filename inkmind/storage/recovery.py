...
class RecoveryManager:
    ...
    async def recover(self, novel_id: UUID) -> RecoveredMemoryState:
        ...
        for archive in archives:
            if archive.tier == "l0_index":
                state.l0_index = L0Index(
                    novel_id=UUID(archive.novel_id), **archive.data
                )
            elif archive.tier == "l2_compressed":
                state.l2_archive = L2Archive(
                    novel_id=UUID(archive.novel_id), **archive.data
                )
            elif archive.tier == "l3_permanent":
                state.l3_archive = L3Archive(
                    novel_id=UUID(archive.novel_id), **archive.data
                )
        ...
        # Step 6: 加载待处理压缩任务
        state.pending_tasks = await self._load_pending_tasks(
            novel_id_str
        )

        # Step 7: 加载 PipelineState
        state.pipeline_state = await self._load_pipeline(
            novel_id_str
        )
    ...
    async def _load_archives(
        self, novel_id: str
    ) -> list[MemoryArchiveModel]:
        result = await self._session.execute(
            select(MemoryArchiveModel).where(
                MemoryArchiveModel.novel_id == novel_id
            )
        )
    ...
    async def _load_pipeline(
        ...
        result = await self._session.execute(
            select(PipelineStateModel).where(
                PipelineStateModel.novel_id == novel_id
            )
        )
