...
- Provider 注册：ProviderConfig（base_url, api_key_env, max_concurrent, max_calls_per_minute）
...
**速率限制（ADR-0010 §10-E）：** per-Provider `RateLimiter`（rate_limiter.py）滑动窗口计数器，`ProviderConfig.max_calls_per_minute` 配置（默认 0 = 不限制），每次 HTTP 尝试前 acquire

**观测层（ADR-0010）：** 每次调用产生不可变 `ProviderStats` 快照（frozen dataclass，含 latency_ms/success/error_type/degraded/retry_count/timestamp），基类 `_record_call()` 统一埋点；`LLMClient.record_stats / aggregate_stats / reset_stats` 会话级聚合；`get_stats()` 返回可变累计器 `ProviderStatsAccumulator`

**涉及文件：**
- `inkmind/models/llm.py` — 配置模型
- `inkmind/llm/client.py` — 统一客户端接口
- `inkmind/llm/factory.py` — 工厂 + 路由 + 降级
- `inkmind/llm/rate_limiter.py` — 速率限制器
- `inkmind/llm/providers/base.py` — 抽象基类（含 Stats 埋点）
...
**并发安全（ADR-0011 §11-C）：**
- `UnitOfWork(session, db_path=...)` 构造时，`commit()` 在 `{db_path}.lock` 文件锁保护下序列化提交；`with uow` 已持锁时不重复获取
- CLI `get_uow(db_path)` 已接线：init / write / review / restore / next / plan 写事务均持锁
- `FileLock.from_path(db_path)` 锁文件与数据库同目录；`aacquire()` 为异步获取（不阻塞事件循环）
