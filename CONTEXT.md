...
**ThreeLevelOutline**:
三级大纲。长篇大纲的三级嵌套体系：总纲（整书主线/结局/核心矛盾）→ 卷纲（每卷的主线/支线/阶段目标/卷末悬念）→ 章纲（每章摘要）。规划顺序自上而下。

**OutlineSpine**:
总纲。Novel 的 1:1 大纲首部，人工所有、就地可改。六字段：主线、核心矛盾、结局、卖点、世界背景、金手指。

**Volume**:
卷。三级大纲的中间层，一等实体。承载卷纲四字段（阶段目标/主线/支线/卷末悬念）、`volume_index` 显式排序（尾部追加）与 `planned_size`（预计章数，软目标，缩卷下限=已排章数）。章节区间派生：起点 = 1 + Σ 前序卷 planned_size，区间 `[起点, 起点+planned_size)`；章行以 `volume_id` 显式归属（NOT NULL），单次规划范围须落单卷区间内。
_Avoid_: 册、集
...
**ChapterOutline**:
章纲。Chapter 的规划态字段集（标题/摘要/关键事件/节奏标记/视角角色/出场角色），Planner 批量产出、人工就地可改。由 Chapter 行直接承载，无独立大纲表。

**RhythmMarker**:
节奏标记。章纲的枚举字段：`none` / `minor_climax`（▲ 小高潮）/ `major_climax`（★ 卷末大高潮）。Planner 规划时产出，人工可改；属创作判断，不做规则派生。

**UnstartedChapter**:
未开工章。`ChapterStatus == PLANNED` 的章——仅有大纲、未产出正文（无版本、无批注）。重规划唯一允许覆盖的对象；其余一切状态（含 `AWAITING_HUMAN` 人工门）皆锁定。

**ChapterStatus**:
Chapter 的状态机：`PLANNED`（已规划）→ `WRITING` → `DRAFT_READY` → `REVIEWING` →（`REVISING` 循环）→ `APPROVED` → `FINALIZED`；另有 `AWAITING_HUMAN` 人工门——一圈流水线跑完后停，待人裁决（提交/批示再修/手动改）。
...
**PipelineState**:
流水线全局状态。跟踪每章状态（见 ChapterStatus）、当前章节与修订迭代。`total_chapters` 为「已规划高水位」（最后已排章号），与全书预计规模（Σ 各卷 planned_size）解耦。
